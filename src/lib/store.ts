...
type Snapshot = Pick<
  AppState,
  'courses' | 'assignments' | 'blocks' | 'sessions' | 'todayList' | 'units'
>
  ...
  persist(
    (set, get) => {
      const mutate = (label: string, fn: (s: AppState) => Partial<AppState>) => {
        ...
        if (nextState.units && unitsTouched) {
          ...
          nextState.assignments = syncAssignmentsFromUnits(nextState.assignments, nextState.units)
      ...
      return {
        ...
        setUnitStatus(id, status) {
          ...
          mutate(status === 'done' ? 'Completed unit' : 'Reopened unit', (s) => {
            ...
            return {
              ...
              sessions: mergeProjectedSessions(relational.sessions, s.sessions, currentUnits),
            }
          })
        },
        ...
        scheduleUnit(id, slot) {
          mutate('Updated schedule', (s) => {
            ...
            return {
              ...
              assignments: relational.assignments,
              blocks: relational.blocks,
              sessions: mergeProjectedSessions(relational.sessions, s.sessions, stateToUnits(s)),
            }
          })
        },
        ...
        createUnit(input) {
          ...
          const parentId = input.parentId && get().getUnits()[input.parentId] ? input.parentId : null
          ...
          mutate('Created work unit', (s) => {
            ...
            return {
              ...
              assignments: relational.assignments,
              blocks: relational.blocks,
              sessions: mergeProjectedSessions(relational.sessions, s.sessions, priorUnits),
            }
          })
          return unit
        },
        ...
        duplicateBlock(id) {
          const { blocks, courses, settings } = get()
        ...
        removeBlock(id) {
          mutate('Deleted block', (s) => ({ blocks: s.blocks.filter((b) => b.id !== id) }))
        },
        toggleBlockDone(id) {
        ...
        reconcileTimer() {
          ...
          set(withRecovery(get(), r) as never)
          return r
        },
        ...
        endSitting(opts = {}) {
          ...
          const priorUnits =
            before.units && Object.keys(before.units).length > 0 ? before.units : stateToUnits(before)
          const syncedSessions = mergeProjectedSessions(rel.sessions, sessions, priorUnits)
          const syncedAssignments = syncAssignmentsFromUnits(nextAssignments, nextUnits)
        ...
        importState(next) {
          ...
          const imported = stateToUnits({ ...nextState, units: next.units ?? {} })
          set({
            ...nextState,
            units: imported,
            assignments: syncAssignmentsFromUnits(nextState.assignments, imported),
    ...
    },
    {
      ...
      merge: (persisted, current) => {
        ...
        next.units = stateToUnits(next)
        next.assignments = syncAssignmentsFromUnits(next.assignments, next.units)
        ...
        recovered.units = stateToUnits(recovered)
        recovered.assignments = syncAssignmentsFromUnits(recovered.assignments, recovered.units)
