...
export const GREETINGS = {
  morning: ['Morning', 'Good morning', 'Hey'],
  afternoon: ['Afternoon', 'Good afternoon', 'Hey'],
  evening: ['Evening', 'Good evening', 'Hey'],
  night: ['Late session', 'Still up?', 'Hey'],
}
...
export const greet = (d = new Date(), seed = '') => {
  const h = d.getHours()
  const pool = h < 12 ? GREETINGS.morning : h < 17 ? GREETINGS.afternoon : h < 22 ? GREETINGS.evening : GREETINGS.night
  return pick(pool, seed + h)
}
