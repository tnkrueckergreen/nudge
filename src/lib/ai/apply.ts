...
export function foldProposal(state: AppState, p: Proposal, nowIso: string): Partial<AppState> {
  switch (p.type) {
    case 'create_task': {
      ...
      return { assignments: [...state.assignments, a] }
    }
    ...
    case 'complete_task': {
      const targetBlockIds = new Set(
        p.done
          ? state.blocks
              .filter((b) => b.assignmentId === p.taskId)
              .map((b) => b.id)
          : [],
      )
      return {
        ...
        blocks:
          p.done && targetBlockIds.size > 0
            ? state.blocks.map((b) => (targetBlockIds.has(b.id) ? { ...b, done: true } : b))
            : state.blocks,
        sessions:
          p.done && targetBlockIds.size > 0
            ? syncBlockSessions(state.blocks, state.sessions, targetBlockIds, true, nowIso)
            : state.sessions,
      }
    }
...
export function applyProposals(proposals: Proposal[], vstate: ValidationState): ApplyResult {
  ...
  useStore.setState((s) => {
    ...
    return {
      assignments: syncAssignmentsFromUnits(working.assignments, units),
      ...
      sessions: mergeProjectedSessions(rel.sessions, working.sessions, s.units ?? {}),
      units,
    }
  })
