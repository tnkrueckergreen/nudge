...
import type { Assignment, Course, Meeting, MeetingKind, StudyBlock } from './types'
...
export function classesOn(courses: Course[], day: Date | number): ClassOccurrence[] {
...
export function nextClass(courses: Course[], now: number, withinDays = 7): ClassOccurrence | null {
  for (let i = 0; i <= withinDays; i++) {
    const day = +startOfDay(now) + i * 86_400_000
    const hit = classesOn(courses, day).find((c) => c.end > now)
    if (hit) return hit
  }
  return null
}
...
export function dayAgenda(
  ...
  const entries: Omit<AgendaEntry, 'hop'>[] = classesOn(courses, day).map((cls) => ({
    id: cls.id,
    start: cls.start,
    end: cls.end,
